...
files.forEach(f => {
    ...
    // Prefix replacements array, starting with more specific replacements first
    // Note: We need to avoid turning "bg-emerald-500" into "bg-success0" if we replace 'emerald-50' first.
    // So we sort the map keys by length descending!
    const sortedMap = Object.entries(map).sort((a, b) => b[0].length - a[0].length);

    ['text-', 'bg-', 'border-', 'ring-', 'fill-', 'stroke-', 'shadow-'].forEach(prefix => {
       sortedMap.forEach(([key, val]) => {
           // Basic string match because regex with dynamic boundaries inside tailwind classes is tricky
           const search = prefix + key;
           const replace = prefix + val;
           // Using regex with boundary to avoid partial match, e.g., 'bg-emerald-5' matching 'bg-emerald-500'
           const re = new RegExp(search.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&') + "(?![0-9])", "g");
